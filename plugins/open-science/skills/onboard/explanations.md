...
This sets up the open-science tools for you. They help you run a research project with an AI
agent so that everything is written down: plans, results, failed attempts and sources. You can
use five parts, in any combination (one of them needs another, which I will explain). I will first check what this computer already has, so I
only ask about what is missing. Nothing in your own settings changes unless you say yes to
that change.
...
## Q1 — which parts (multi-select)

Question: "Which parts do you want to use? You can pick any combination, and add others
later by running this again."
- **Publish projects**: Each project gets two copies. The private one is where you and the
  agent work; it can hold drafts, notes and data. The public one is what the world sees. You
  never copy files to it by hand: when you ask to publish, the agent collects only the files
  you allowed in a list, checks them for passwords, private paths and anything you listed as
  private, and shows you a report. Nothing goes public until you approve that report. The
  public copy also gets its own web page on GitHub, and data can be archived on Zenodo with a
  DOI (a permanent citable link). Works with any git repository. Needs a free GitHub account.
- **Project structure**: A standard layout for each research project, so that you and every
  agent find things in the same place: a description of the project, one folder per task
  with its plan, where it stands and its history, a map of how the tasks depend on each
  other, the sources used, and written rules the agents follow. Agents keep these records
  current as they work, so any new session can pick up the work. Works with or without the
  other parts; with publishing, each task's record also says whether it may go public, and
  the publish check also looks at the map and the records.
...
- **Context management**: An AI agent can only hold a limited amount of conversation. On long
  ...
  your answer. **Needs the project structure part** (the records it resumes from) and tmux,
  a program that keeps terminal windows alive and lets the agent type into its own window.

If the user picks context management without project structure, say that it needs it and
ask: "add project structure too" or "leave context management out".

## Q1b — cluster time limits (ask only if the check found SLURM)

Question: "This computer uses the SLURM job scheduler. Do you also want your agent sessions
to survive the time limit of a SLURM job?"
- **Yes**: A job stops at its time limit, which would end your agent sessions. With this
  part, before the limit the agents are told to finish cleanly, a new job is queued, and when
  it starts your tmux windows are rebuilt and every agent session continues where it
  stopped. Needs tmux, with Claude running inside a SLURM batch job. Works with or without
  the other parts.
- **No**: nothing is installed for SLURM.

## Branch 1 — publishing

**1b — how git reaches GitHub** (skip if the SSH check says ok)
...
**1c — who creates each public repository**
...
**1d — where the private copy lives**
...
**1e — the web page** (no question): after the first publish, open the public repository on
github.com → Settings → Pages → Source: "GitHub Actions". Then every publish rebuilds the page.

**1f — Zenodo**: "Zenodo is a free archive run by CERN. It gives a dataset a DOI, a permanent
...
## Branch 3 — context management

**3a — tmux** (skip if already inside tmux): explain: "tmux keeps your terminal windows
...
**3b — older skills with the same names** (skip if the check found none)
