...
5. **Check.** With the user's yes, write
   ...
   in the workspace (`user_candidate=` lines). Ask which of them is the user, unless there
   is only one, then add `user: <id>` under `notion`. Run the check again; every line must
   end in `ok`.
