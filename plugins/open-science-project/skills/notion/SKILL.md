---
name: notion
description: Work in a project that is mirrored to Notion - keep the Notion pages in sync with the project files, keep task plots (with self-contained captions) current in the task pages, post results, questions, blockers and status to the project's Notion Feed, and set up the mirror for a new or existing project. Use when a project's AGENTS.md has a "Notion" section, when the owner asks to sync, update or show something in Notion, to remake or update plots in Notion, to post or notify through Notion, or to mirror a project to Notion.
---

# Notion

A project with `notion: true` in `config/framework.yaml` is mirrored to Notion, and the owner
reads it there. `opsci notion` writes the pages as the owner's Notion integration (a bot), so
an @mention from it notifies the owner. Full reference: `docs/notion.md` in the framework
repo.
...
- The **Feed**: messages to the owner, newest first, removed after 3 days.
...
5. **A remade plot gets a new dated name** (`bands_2026-09-28.png` after
   ...
   When the owner asks to update or remake plots: make the new files and captions, run
   `opsci notion sync --plots-only`, then check `opsci notion diff`.
6. **Post to the Feed** at the points the owner cares about:

   | when | command |
   |---|---|
   | a subtask finished, with its result | `opsci notion post --kind result --task <id> --mention --file <key plot> "<title>\n\n<result with numbers>"` |
   | a question or decision for the owner | `--kind question --mention` |
...
- **New project:** `open-science-project:new-project` does it when `opsci notion check`
  ...
  `opsci notion init`.
- **Existing project** (the owner asks to mirror it): run `opsci notion check`. If the token or
  parent page is missing, send the owner to `open-science:onboard` (Notion setup). Then run
...
- `config/notion.local.yaml` holds the page ids (git-ignored). If it is lost, the pages
  cannot be found again: say so; do not run `init` a second time without the owner.
...
| `NOTION_TOKEN missing`, `credentials file not found` | the owner runs the token step of `open-science:onboard` |
| `group or others have access` | the owner runs `chmod 600` on the token file |
| `object_not_found` on the parent page | the owner shares the page with the integration (••• → Connections) |
