...
3. **Propose a mapping and get the user's approval before moving anything.** Ask the user
   ...
   goes under `hard_private:` in `publish/manifest.yaml`. Unless the user declined results
   (step 2), the mapping also lists each task's results: title, kind, the file that shows
   it, and whether you propose it as a milestone (ask when unsure). Write the approved
