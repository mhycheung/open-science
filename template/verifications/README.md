...
names the nodes it checks in `verifies:`. The graphs draw it as a card with a double
border, with a dashed "verified by" arrow from each node it verifies. Its privacy is by default the
