...
- **Record the work.** A request for work in this project (an idea to explore, a question to
  ...
  records it. Never make the user say "task" or "not a task" before they get an answer. A
...
| `tasks/<id>/` | one task: `context.md` (node header), `plan.md`, `map.md` (the task's graph), `results/` (its scientific results, one file each), `log.md`, `subcontext/`, working files | that task's agents |
