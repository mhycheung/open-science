...
- check: the token file, the integration, the parent page, and the owner's Notion user.
...
This project is mirrored to Notion, where the owner reads it (skill
...
- **Feed.** Post with `opsci notion post --kind KIND [--task ID] [--mention] [--file PLOT]
  ...
  blocker for the owner as `question` or `blocker` with `--mention`; a long job submitted
...
def send_test(project_root: str | None = None, out=print) -> int:
    """Make a page under the parent page that @mentions the owner (onboarding's test)."""
    proj = Project(project_root)
    parent, owner = proj.section.get("parent_page"), proj.section.get("owner")
    if not parent or not owner:
        raise NotionError("set notify.notion.parent_page and notify.notion.owner first (opsci notion check)")
    page = proj.client.call("POST", "/pages", {
        ...
        "children": [nb.blk("paragraph", [{"type": "mention", "mention": {"type": "user", "user": {"id": owner}}}]
...
def check(project_root: str | None = None, out=print) -> int:
    ...
    if not parent:
        ...
        try:
            ...
            ok = False
    owner = proj.section.get("owner")
    if owner:
        try:
            u = c.call("GET", f"/users/{owner}")
            out(f"owner=ok:{u.get('name')}")
        except NotionError as exc:
            out(f"owner=problem: {exc}")
            ok = False
    else:
        ...
        for u in people:
            ...
            out(f"owner_candidate={u['id']} {u.get('name', '')} {email}".rstrip())
        ok = False
