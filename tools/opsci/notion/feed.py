"""The project's Feed page: messages to the owner, newest first, removed after a few days.
...
(<stem>.caption.md). With `mention`, the owner (notify.notion.owner) is @mentioned, so Notion
notifies them: the messages are written by the integration (a bot), not by the owner.
...
def post(proj: Project, text: str, author: str = "agent", kind: str = "note", task: str = "",
    ...
    if mention and not owner:
        raise NotionError("no owner to mention: set notify.notion.owner (opsci notion check prints it)")
    ...
    head = []
    if mention:
        head += [{"type": "mention", "mention": {"type": "user", "user": {"id": owner}}}] + nb._t(" ")
