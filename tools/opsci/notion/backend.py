...
Config: `notify: {backend: notion, notion: {parent_page, owner, kind, mention, author}}`.
`kind` (default note) and `mention` (default true: the owner is @mentioned, so Notion notifies
    ...
    def send(self, text: str, attachment: Path | None) -> str:
        ...
        try:
            feed.post(proj, text, author=os.environ.get("OPSCI_AUTHOR") or self.section.get("author") or "agent",
                      kind=self.section.get("kind", "note"), mention=bool(mention) and bool(proj.section.get("owner")),
