...
def write_config(path: Path, mock: MockNotion, creds: Path, backend="notion", owner=True):
    section = {"credentials_file": str(creds), "parent_page": mock.parent_id}
    if owner:
        section["owner"] = mock.person["id"]
...
def test_blocks_deleted_by_hand_in_notion(mirrored):
    """A plot block the owner already deleted in Notion: removing the plot still syncs."""
    S, m = mirrored, mirrored.mock
    (e,) = plot_entries(m, task_page(S.proj))
...
def test_feed_post_newest_first_and_prune(mirrored):
    ...
    # the result: an icon, a mention of the owner, the bold title
    ...
    # a message the owner already deleted in Notion is pruned without error
...
def test_check_ok(S):
    ...
    assert r.stdout.splitlines() == ["backend=notion", "token=ok", "integration=ok:opsci test bot",
                                     "parent_page=ok:Research", "owner=ok:Owner Person"]


def test_check_lists_owner_candidates(S):
    write_config(S.cfg, S.mock, S.creds, owner=False, backend="file")
    r = run(S, "notion", "check", cwd=make_project(S), rc=1)
    lines = r.stdout.splitlines()
    assert lines[0] == "backend=file"
    assert "owner=missing" in lines
    assert f"owner_candidate={S.mock.person['id']} Owner Person owner@example.org" in lines
...
def test_real_notion_check():
    """The user's real config and token; only reads (users/me, the parent page, the owner)."""
