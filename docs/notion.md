...
4. **Config** in `~/.config/opsci/config.yaml` (not secret):
   ...
   notify:
     ...
     notion:
       ...
       owner: 01234567-89ab-cdef-0123-456789abcdef   # your Notion user id; opsci notion check lists them
       # credentials_file: ~/.config/opsci/notion.env   (the default)
   ```

5. **Check:** `opsci notion check` prints `backend=`, `token=`, `integration=`,
   `parent_page=` and `owner=` lines. `opsci notion check --send-test` makes a page that
