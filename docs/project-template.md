...
| `tasks/<id>/` | one directory per task: `context.md` (node header and state), `plan.md`, `log.md`, `subcontext/`, working files | that task's agents |
...
- After changing a header, run `opsci map build`. It writes `map/graph.md` (a Mermaid graph
  and a table of every node) and `map/dead_ends.md` (every failed, superseded or abandoned
...
| `status_exempt` | markdown files that need no `status:` header; replaces the default list (READMEs, `AGENTS.md`, `CLAUDE.md`, `PROJECT.md`, `context.md`, `log/`, `map/`, `citations/`, `rules/`, task logs and `subcontext/`, `docs/`, and the same skeleton files under `brainstorm/`) |
